      ...
      if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
        ...
        if (way >= 0) { // read hit
          ...
          if (RQ.entry[index].type == LOAD) {
            if (cache_type == IS_L1D) 
              // only the L1 cache has access to the Read Queue and the Write Queue (that are actually inside the processor, but the L1 reads the requests directly from the queues)
              // that's why the L2 and LLC caches just access directly their blocks
              // and that's why the address used for prefetching in the L1 is the full address, while for the L2 and LLC caches there is this weird modification
    ...
    int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata)
    {
      ...
      if (PQ.occupancy < PQ.SIZE) {
        ...
        if ((base_addr>>LOG2_PAGE_SIZE) == (pf_addr>>LOG2_PAGE_SIZE)) {
          ...
          // the packet being inserted at the prefetcher queue
          // give a dummy 0 as the IP of a prefetch
          add_pq(&pf_packet);

          // good metric to take into account TODO
          pf_issued++;
